...
#### Phase 4 — Mature
- Establish a vendor SBOM quality baseline: SBOM delivered with each release, in a supported format, with VEX statements issued for disclosed vulnerabilities. Factor compliance against this baseline into renewal decisions.
- Evaluate whether your asset inventory and SBOM storage approach should advance to a structured ITAM platform or dedicated SBOM management tool appropriate to your current scale. See [SBOM Tooling](../tooling).
- If operating in a regulated sector, confirm that your SBOM records and vendor SBOM documentation satisfy current examination or audit requirements.
...
- Stand up a receiving stack. Ingest first-party SBOMs first to validate the pipeline before onboarding vendor SBOMs. See [SBOM Tooling](../tooling) for current options.
- Identify the top 10–20 critical (Tier 1) vendors and begin outreach. Do not onboard the long tail until the Tier 1 pipeline is stable.
- Build asset-record linkage: SKU-level for now, instance-level for Tier 1.

#### Phase 2 — Operationalize
- Onboard Tier 1 and Tier 2 vendors. Capture SBOMs across at least one full release cycle before drawing conclusions about coverage.
- Correlate SBOM inventory against vulnerability management. KEV-first triage. Begin VEX/CSAF intake from vendors who provide it.
- Engage the OSPO and legal team on license compliance. Build the working relationship for ongoing review.
- Connect ITAM/SAM records. Ensure SBOM linkages align with procurement records.

#### Phase 3 — Scale
- Onboard Tier 3 vendors. Apply tier-appropriate contract clauses; do not over-specify for low-criticality suppliers.
- For vendors that cannot or will not produce SBOMs, generate analyzed SBOMs internally using tooling such as Syft or Trivy against their binaries or containers. Tag consumer-generated SBOMs clearly — including generation method and date — to distinguish them from producer-issued build SBOMs.
...
- Evaluate whether your scope should extend to non-software BOMs relevant to your environment. See [Connected SBOM Artifacts](../companion-artifacts) for the landscape.
